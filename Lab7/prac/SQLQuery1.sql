...
--where [User].name=@nam
--end
----declare @identity varchar(20)='Ali'
--exec dbo.pc 
--@nam='Ali'

--execute pc @nam='Ali'
----Question 3
--create procedure dalla
--@num varchar(20)
--as
--begin
--select name,phonenum,city from [User] us
--join [UserCard] u on us.userId=u.userID 
--where u.cardNum=@num
--end
--execute dalla
--@num='1324327436569'
...
--as 
--begin
--select min(balance) from [Card]
--end
--declare @printer float
--exec dbo.mc
--@bal=@printer
----Question 5
--create procedure pehnyakka
--@name varchar(20),
--@uid int,
--@num int output
--as
--begin
--select count(us.userID) as NumOfCard 
--from [UserCard] us join [User] u on u.userId=us.userID
--where u.userID=@uid and u.name=@name
--end
--declare @phuddika int 
--execute pehnyakka 
--@name='Ali',
--@uid=1,
--@num=@phuddika 
----Question 6
--create procedure tarnel1
--@c varchar(20),
--@p varchar(4),
--@status int output
--as
--begin 
--if exists(select cardnum,pin from [Card] c
--where c.cardNum=@c and c.PIN=@p)
--	begin
--		set @status=1
--	end
--else
